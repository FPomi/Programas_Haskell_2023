...
--Comprueba si los elementos de elementos1 existen en elementos2
elementosPertenecen :: (Eq elemento) => [elemento] -> [elemento] -> Bool
elementosPertenecen elementos1 elementos2   | elementos1 == [] = True
                                            | pertenece (head elementos1) elementos2 = elementosPertenecen (tail elementos1) elementos2 
                                            | otherwise = False

--Compara los elementos de dos conjuntos para ver si son iguales
mismosElementos :: (Eq elemento) => [elemento] -> [elemento] -> Bool
mismosElementos elementos1 elementos2 = (elementosPertenecen elementos1 elementos2) && (elementosPertenecen elementos2 elementos1)
...
filtrarPorUsuario :: [Publicacion] -> Usuario -> [Publicacion]
filtrarPorUsuario ps u  | ps == [] = []
                        | usuarioDePublicacion (head (ps)) == u = [head ps] ++ filtrarPorUsuario (tail ps) u
                        | otherwise = filtrarPorUsuario (tail ps) u
...
tieneUnSeguidorFiel (us, rs, ps) u  | (publicacionesDe (us, rs, ps) u == [] || us == []) = False
                                    | ((head(us) /= u) && (mismosElementos (filtrarPorUsuario (publicacionesQueLeGustanA (us, rs, ps) (head us)) u) (publicacionesDe (us, rs, ps) u))) = True
